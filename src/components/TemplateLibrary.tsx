...
export const TemplateLibrary: React.FC<TemplateLibraryProps> = ({ isOpen, onClose, selectedPatient }) => {
  ...
  const replacePlaceholders = (content: string) => {
    ...
    return content
      ...
      .replace(/\[Doctor Name\]/g, selectedPatient.doctor || 'the doctor')
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="flex border-b border-slate-100">
          <button
            ...
            className={`flex-1 py-3 text-sm font-semibold transition-colors ${
              activeTab === 'internal' 
                ? 'text-clinic-teal border-b-2 border-clinic-teal bg-slate-50' 
        ...
        {activeTab === 'internal' ? (
          ...
          <div className="flex-1 bg-slate-50 relative">
            <iframe 
              src="https://clinic-templates-isihat.vercel.app/" 
              className="w-full h-full border-none"
              title="iSihat External Library"
            />
            <div className="absolute bottom-4 right-4">
              <a 
                ...
                className="bg-white shadow-lg border border-slate-200 px-4 py-2 rounded-full text-xs font-bold text-clinic-teal hover:bg-slate-50 transition-all flex items-center gap-2"
              >
                Open in New Tab
              </a>
            </div>
