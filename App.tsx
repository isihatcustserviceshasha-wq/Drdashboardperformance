...
export default function App() {
  ...
  // Doctor CRUD
  const handleCreateDoctor = async (name: string, specialty?: string) => {
    try {
      const { data, error } = await supabase
        .from('doctors')
        .insert([{ name, specialty, is_active: true }])
        .select();
  ...
  const performanceData = useMemo(() => {
    ...
    const doctorsWithRecords = Array.from(new Set(outcomes.map(o => o.doctor)));
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Cloud Database</p>
                    <p className="text-xs font-semibold text-slate-700">Supabase Connected</p>
                  </div>
                </div>
              </div>
